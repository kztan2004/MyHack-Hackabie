  ...
  return (
    <section>
      <div className="mb-4 flex items-end justify-between gap-3">
        <div>
          <h1 className={`${compact ? "text-xl" : "text-2xl"} font-semibold text-ink`}>Linkage Graph</h1>
          <p className="mt-1 text-sm text-slate-500">{graph.nodes.length} nodes, {graph.edges.length} linkages</p>
        </div>
      ...
      >
        ...
        {!filteredGraph.nodes.length && (
          <div className="absolute inset-0 flex items-center justify-center text-slate-500 font-medium bg-[#0f172a]">
            {search ? "No matching linkages found." : "No graph linkages yet."}
          </div>
        )}
